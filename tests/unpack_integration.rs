...
fn temp_dir(name: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("sure-unpack-test-{}-{name}", std::process::id()));
...
fn create_gz(dir: &Path) -> PathBuf {
    ...
    let status = Command::new("gzip").arg("-k").arg(&src).status().unwrap();
    assert!(status.success());
    dir.join("hello.txt.gz")
}

fn create_lz4(dir: &Path) -> PathBuf {
    let src = dir.join("hello.txt");
    fs::write(&src, "hello world\n").unwrap();
    let archive = dir.join("hello.txt.lz4");
...
fn create_tar_lz4(dir: &Path) -> PathBuf {
    let src_dir = dir.join("src");
    fs::create_dir_all(src_dir.join("mydir")).unwrap();
    fs::write(src_dir.join("mydir/hello.txt"), "hello world\n").unwrap();
    fs::write(src_dir.join("mydir/bye.txt"), "goodbye\n").unwrap();
...
fn extract_lz4_single() {
    ...
    let dir = temp_dir("extract-lz4");
    let archive = create_lz4(&dir);
    let work = dir.join("work");
    fs::create_dir_all(&work).unwrap();
...
fn extract_tar_lz4() {
    ...
    let dir = temp_dir("extract-tar-lz4");
    let archive = create_tar_lz4(&dir);
    let work = dir.join("work");
    fs::create_dir_all(&work).unwrap();
...
#[test]
fn sniff_detects_tar_lz4() {
    if !has_tool("tar") || !has_tool("lz4") {
        return;
    }
    let dir = temp_dir("sniff-tar-lz4");
    let archive = create_tar_lz4(&dir);
    let renamed = dir.join("mystery.bin");
    fs::rename(&archive, &renamed).unwrap();
...
fn sniff_subcommand_unknown() {
    ...
    let output = sure_unpack().arg("sniff").arg(&f).output().unwrap();
    assert!(!output.status.success());
...
fn sniff_subcommand_renamed() {
    ...
    let output = sure_unpack().arg("sniff").arg(&renamed).output().unwrap();
    assert!(output.status.success());
